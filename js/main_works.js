var NUM_PALETTES = 8;
var SEISMOMETER_DATA_SERVER = "http://etch-1169-3.ieor.berkeley.edu/naturalFrequencies/naturalFrequencies.py";
var NUM_RINGS_PER_BLOOM = 16;
...
function createBloom(counter, iParam) {
    ...
    for (i = 0; i < numRings; i++) {
        var randomX = getRandomInt(0, getImgWidth(randomPaletteIndex));
        var randomY = getRandomInt(0, getImgHeight(randomPaletteIndex));
    ...
    for (i = numRings - 1; i >= 0; i--) {
        ...
        */
        var max_radius = 10 + Math.abs(smoothedData[iParam] / 10);
        circle_array.push(new g_circle_t(counter, x, y, init_r, 10 + max_radius * (i + 1) / numRings, rgbColors[i].r, rgbColors[i].g, rgbColors[i].b, max_radius * 2));
    }
...
function g_circle_t(counter, x, y, min_r, max_r, red, green, blue, max_time) {
...
function g_circle_draw(g_circle) {
    ...
    // ctx.fill();
    ctx.lineWidth = g_circle.max_r / 30;
    ctx.stroke();
}
...
function update_circle_state(g_circle) {
    ...
    if (g_circle.is_fading_away) {
        ...
        g_circle.r = g_circle.max_r * Math.log(1 + (g_circle.time / 5));
